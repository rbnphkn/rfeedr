...
function _eventsBindings() {
  ...
  $("#settings").on('hidden', function() {
    if (sessionStorage.sourceDirty) {
      sessionStorage.removeItem('sourceDirty');
      location.reload();
    }
  });
}
...
function _cleanupRoutine() {
  var olddate = (new Date()).getTime() - CLEANUP_LIMIT;
  _db_.transaction(function (tx){
    tx.executeSql('DELETE FROM posts WHERE date <= ' + olddate );
  });
  setTimeout(_cleanupRoutine, 1000 * 60 * 60 * 24); // Run every 24 hours
}
...
function _drawPosts() {
  _db_.transaction(function (tx) {
    ...
    tx.executeSql('SELECT * FROM posts LEFT OUTER JOIN sourceList ON posts.sourceid = sourceList.id WHERE posts.date < ' + _lastDraw_ + ' AND sourceList.status = "YES" ORDER BY posts.date DESC LIMIT ' + totalItems, [], function (tx, results) {
...
function _drawDiv(conf, pagePosts, singlePost) {
  ...
  var elems = "<div class='f-page'>" +
      ...
      "<h2>RFeedR<img class='spinner' src='./images/spinner-small.gif' style='display:none;'></h2>" +
              ...
              "</div>";
  if (singlePost) {
    elems += "<div class='wcent'><div class='closePage'>Close</div><div class='forwardThis' data-url='" + pagePosts[0].url + "' data-title='" + pagePosts[0].title + "'>Forward</div></div>";
  }
